...
interface User {
  ...
  email: string;
  isPremium: boolean;
  // FIX: Added optional gender field to the User interface
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signup = async (username: string, email: string, password: string) => {
    const response = await fetch(`${API_URL}/auth/signup`, {
      ...
      // Note: Gender is not sent here. Will fix the signup page next.
    ...
    if (!response.ok) {
      ...
      try {
          ...
          throw new Error(text || "Server responded with an unknown error.");
      }
    ...
    const data = await response.json();
    setToken(data.token);
    // FIX: Ensure 'gender' is present on the object returned from signup if the server handles it.
    // If the server doesn't return 'gender', it remains optional (undefined), which is handled by the interface.
    setUser(data.user); 
    localStorage.setItem("echochat_token", data.token);
  };
